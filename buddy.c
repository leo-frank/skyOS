...
buddy_pages_init() {
    ...
    for (int i = 0; i < npage; i += BUDDY_MAX_ORDER_NR_PAGES) {
        ...
        list_append(&(pages[i].list_link), &(buddy_lists[BUDDY_MAX_ORDER-1].head));
 ...
 */
static void *
buddy_split(uint order_large, uint order_small) {
...
void 
buddy_free_page(struct Page* p) {
    if (p->flag == PG_TAIL)
        panic("[buddy]: free a tail page, this is not allowed.\n");
    if (p->free == 1)
        panic("[buddy]: free a free page.\n");
    struct Page * buddy_p;
    struct list_head *lh;
    ...
    while (p->order < (BUDDY_MAX_ORDER-1)) {
        ...
        if (buddy_p != NULL && buddy_p->free == 1 &&
            buddy_p->flag == PG_HEAD && buddy_p->order == p->order) {
