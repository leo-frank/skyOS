...
static inline uint32 fat_eoc(const struct fat_vol_handle *fat) {
  switch (fat->type) {
    case FAT_TYPE_FAT12:
      return 0x0FF8;
    case FAT_TYPE_FAT16:
...
static inline uint32 fat_first_sector_of_cluster(
    const struct fat_vol_handle *fat, uint32 n) {
...
#define FAT_FLUSH_SECTOR()                                            \
  do {                                                                \
    if (_fat_cache.dirty)                                             \
      if (block_write_sectors(_fat_cache.bldev, _fat_cache.sector, 1, \
                              _fat_sector_buf) != 1)                  \
        return -EIO;                                                  \
    _fat_cache.dirty = 0;                                             \
  } while (0)

#define FAT_GET_SECTOR(fat, sectorn)                                          \
  do {                                                                        \
...
#define FAT_PUT_SECTOR(fat, sectorn)                                          \
  do {                                                                        \
    if ((_fat_cache.bldev != (fat)->dev) || (_fat_cache.sector != (sectorn))) \
      FAT_FLUSH_SECTOR();                                                     \
                                                                              \
