 ...
 */
uint32 fat_lseek(FatFile *file, uint32 offset, int whence);
...
struct dirent {
  ...
  /* Non-standard */
  uint8 fat_attr;     /**< FAT file attributes, non-standard */
...
struct fat_vol_handle {
  ...
  union {
    ...
    struct {
      uint16 root_sector_count;
      uint16 root_first_sector;
    } fat12_16;
  };
  /* Internal state */
  uint32 last_cluster_alloc;
