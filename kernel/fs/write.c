...
static uint32 fat_find_free_cluster(struct fat_vol_handle *h) {
  uint32 i;
...
static int fat32_set_next_cluster(const struct fat_vol_handle *h, uint8 fat,
                                  uint32 cluster, uint32 next) {
  ...
  next &= 0x0FFFFFFF;
  next |= __get_le32((uint32 *)(_fat_sector_buf + offset)) & 0xF0000000;
  __put_le32((uint32 *)(_fat_sector_buf + offset), next);
...
static int fat16_set_next_cluster(const struct fat_vol_handle *h, uint8 fat,
                                  uint16 cluster, uint16 next) {
  uint32 offset = cluster * 2;
  uint32 sector;
  ...
  FAT_GET_SECTOR(h, sector);
  __put_le16((uint16 *)(_fat_sector_buf + offset), next);
  FAT_PUT_SECTOR(h, sector);

  return 0;
}

static int fat12_set_next_cluster(const struct fat_vol_handle *h, uint8 fat,
                                  uint16 cluster, uint16 next) {
  ...
  if (offset == (uint32)h->bytes_per_sector - 1) {
    ...
    if (cluster & 1) {
      next <<= 4;
      next |= __get_le16((uint16 *)(_fat_sector_buf + offset)) & 0xF;
    } else {
      next &= 0x0FFF;
      next |= __get_le16((uint16 *)(_fat_sector_buf + offset)) & 0xF000;
    }
    __put_le16((uint16 *)(_fat_sector_buf + offset), next);
  }
  FAT_PUT_SECTOR(h, sector);
...
static int fat_set_next_cluster(const struct fat_vol_handle *h, uint32 cluster,
                                uint32 next) {
  int ret = 0;
...
static int fat_alloc_next_cluster(struct fat_vol_handle *h, uint32 cluster,
                                  int clear) {
  /* Return next if already allocated */
  uint32 next = _fat_get_next_cluster(h, cluster);

  if (next != fat_eoc(h)) return next;
  ...
  if (clear) {
    /* Zero new cluster */
    uint32 sector = fat_first_sector_of_cluster(h, next);
    FAT_FLUSH_SECTOR();
    ...
    for (int i = 0; i < h->sectors_per_cluster; i++) {
       ...
       */
      int discard = block_write_sectors(h->dev, sector + i, 1, _fat_sector_buf);
      (void)discard;
    }
  }
...
int fat_write(struct fat_file_handle *h, const void *buf, int size) {
  ...
  for (i = 0; i < size;) {
    ...
    if ((sector % h->fat->sectors_per_cluster) == 0) {
      /* Go to next cluster... */
      uint32 next_cluster =
          fat_alloc_next_cluster(h->fat, h->cur_cluster, h->size == 0);
  ...
  if (h->dirent_sector && (h->position > h->size)) {
    /* Update directory entry with new size */
    h->size = h->position;
    if (!(h->flags & O_ASYNC)) fat_file_sync(h);
  }

  return i;
}

static int fat_chain_unlink(const struct fat_vol_handle *vol, uint32 cluster) {
  int ret = 0;
  while (cluster && (cluster != fat_eoc(vol))) {
    uint32 next = _fat_get_next_cluster(vol, cluster);
...
int fat_unlink(struct fat_vol_handle *vol, const char *name) {
  ...
  if (!h.dirent_sector) return -EISDIR;
...
static void build_short_name(uint8 *sname, const char *name, int n) {
  int i, j;
  ...
  if (suffix)
    for (j = 1; (j < 4) && suffix[j]; j++) sname[j + 7] = toupper(suffix[j]);

  if (((i == 8) && (name[i] != '.')) || ((suffix - name) != i)) {
    if (i > 6) i = 6;
    sname[i] = '~';
    sname[i + 1] = '0' + (n % 10);
  }
...
int _fat_dir_create_file(struct fat_vol_handle *vol, const char *name,
                         uint8 attr, struct fat_file_handle *file) {
  /* Check if file already exists */
  if (!fat_open(vol, name, 0, file)) return -1; /* File exists */
  ...
  for (int i = 1; i < 10; i++) {
    ...
    if (fat_open(vol, (char *)sname, 0, file)) break; /* We have a winner */
    sname[0] = ' ';
  }

  if (sname[0] == ' ') return -1; /* Couldn't find a short name */
  ...
  for (int i = strlen(name) / 13; i >= 0; i--) {
    ...
    int j;
    for (j = 0; j < 5; j++) __put_le16(&ld.name1[j], name[i * 13 + j]);
    for (j = 0; j < 6; j++) __put_le16(&ld.name2[j], name[i * 13 + j + 5]);
    for (j = 0; j < 2; j++) __put_le16(&ld.name3[j], name[i * 13 + j + 11]);
    ld.checksum = _fat_dirent_chksum(sname);
    if (fat_write(&vol->cwd, &ld, sizeof(ld)) != sizeof(ld)) return -1;
  }
  ...
  if (attr == FAT_ATTR_DIRECTORY) {
    ...
    uint32 cluster = fat_find_free_cluster(vol);
    if (!cluster) return -1;
...
int fat_mkdir(struct fat_vol_handle *vol, const char *name) {
  ...
  uint32 sector = fat_first_sector_of_cluster(vol, dir.first_cluster);
